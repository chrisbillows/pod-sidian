...
class OtterApi:
    ...
    def login(self):
        ...
        login_response = self.session.get(f'{API_BASE_URL}/login', 
                                        params={"username": self.email}, 
                                        headers={'authorization': f'Basic {base64.b64encode(f"{self.email}:{self.password}".encode()).decode()}', 
    ...
    def get_speeches(self):
        response = self.session.get(f'{API_BASE_URL}/speeches', params={"userid": self.user['id']})
    ...
    def get_speech(self, speech_id):
        response = self.session.get(f'{API_BASE_URL}/speech',
                                    params={"speech_id": speech_id, "userid": self.user['id']})
        return response.json()['speech']

    def speech_search(self, query):
        response = self.session.get(f'{API_BASE_URL}/speech_search',
